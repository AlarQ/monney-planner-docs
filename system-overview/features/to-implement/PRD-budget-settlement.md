...
**Migration**:
```sql
-- Migrate existing processed_transactions to state-based approach
-- Step 1: Add transaction detail columns
...
ADD COLUMN transaction_status VARCHAR(20) NOT NULL DEFAULT 'created' CHECK (transaction_status IN ('created', 'updated', 'deleted'));

-- Step 2: Change PRIMARY KEY from event_id to transaction_id
-- First, migrate existing data to have one row per transaction (keep latest state per transaction_id)
-- Keep event_id column for idempotency (but not as PRIMARY KEY)
-- Then drop old PRIMARY KEY and create new one
ALTER TABLE processed_transactions DROP CONSTRAINT processed_transactions_pkey;
ALTER TABLE processed_transactions ADD PRIMARY KEY (transaction_id);

-- Step 3: Add UNIQUE constraint on event_id for idempotency
ALTER TABLE processed_transactions ADD CONSTRAINT unique_event_id UNIQUE (event_id);
...
**Implementation**:
- **Technology**: Tokio async runtime with cron-like scheduling
...
**Domain Service: `BudgetSettlementService`**
...
**Service Structure**:
```rust
pub struct BudgetSettlementService {
    budget_repository: Arc<dyn BudgetRepository>,
    summary_repository: Arc<dyn BudgetSummaryRepository>,
    transaction_service_client: Arc<TransactionServiceClient>,
}

impl BudgetSettlementService {
...
### Phase 1: Backend Infrastructure (Week 3-4)
...
**Tasks**:
1. Create database migration for `budget_period_summaries` table
2. Create database migration for `budget_period_summary_categories` table (normalized approach)
3. Create database migration for `settlement_failures` table (error tracking)
4. Create database migration to expand `processed_transactions` table (add transaction details columns: amount, description, date, category_id, category_name, transaction_type, transaction_status) - see TR-1 lines 426-455. **Current state**: Table exists with basic columns (`event_id`, `event_type`, `processed_at`, `user_id`, `transaction_id`). Migration will add transaction detail columns.
5. Update Kafka event handlers to populate transaction details in `processed_transactions` when processing events (INSERT with status='created', INSERT with status='updated', INSERT with status='deleted')
6. Create database migration for `is_recurring` and `current_period_number` columns in `budgets` table
   - Add `is_recurring BOOLEAN NOT NULL DEFAULT false` column
   - Add `current_period_number INTEGER NOT NULL DEFAULT 1` column
   - Set default `is_recurring = false` for all existing budgets (one-time budgets)
7. Define domain models with validation (`BudgetPeriodSummary`, `CategoryBreakdown`, `BudgetInsight`)
8. Implement repository with CRUD operations (`BudgetSummaryRepository`)
9. Build summary generation service (`BudgetSettlementService`)
10. Implement transaction aggregation logic (query `processed_transactions` table filtered by date range and transaction_status - state-based approach with one row per transaction, no DISTINCT ON needed due to state-based design and transaction update constraint)
11. Implement category breakdown calculation
12. Implement period calculation logic (monthly/weekly/yearly with edge cases: month-end dates, leap years, timezone handling)
13. Create scheduled job with error handling
...
- Domain models (`BudgetPeriodSummary`, `CategoryBreakdown`, `BudgetInsight`)
- Repository layer (`BudgetSummaryRepository`)
- Summary generation service (`BudgetSettlementService`)
...
- [ ] Summary generation service generates accurate summaries
- [ ] Scheduled job detects ended periods correctly
- [ ] Period calculations handle edge cases (month-end, leap years)
...
**Acceptance Criteria**:
- [ ] All API endpoints implemented and working (Budget Service)
- [ ] All BFF proxy routes implemented and working
- [ ] BFF correctly forwards requests to Budget Service with JWT tokens
- [ ] BFF correctly handles and normalizes errors from Budget Service
- [ ] Frontend can successfully call all endpoints through BFF
- [ ] Authentication/authorization working correctly (session-based frontend → BFF, JWT BFF → Budget Service)
- [ ] Error handling returns appropriate status codes (both Budget Service and BFF)
- [ ] Request/response model mapping working correctly in BFF
- [ ] OpenAPI spec updated (Budget Service and BFF)
- [ ] All integration and E2E tests passing (including BFF integration tests)
...
## Critical Design Decisions Required Before Implementation

⚠️ **All critical decisions have been made** - Ready to proceed to Phase 1 after Phase 0.5 completion.

**Note**: Race condition mitigation (concurrent job execution) is deferred to future enhancement. MVP will rely on database unique constraints to prevent duplicate summaries.
...
### Overall Completion: ~25% (Phase 0: 100% complete, Phase 0.5: 100% complete)

### ✅ Completed
- **Phase 0: Kafka Event Handling (100%)**
  - Kafka consumer infrastructure (rdkafka integration, consumer service, configuration)
  - Transaction created event handler (src/domain/budget.rs:418-534)
  - Transaction updated event handler (src/domain/budget.rs:537-662)
  - Transaction deleted event handler (src/domain/budget.rs:665-780)
  - Event deduplication and idempotency (atomic event claiming)
  - Consumer monitoring and logging
  - Comprehensive integration tests (1425 lines in tests/integration/kafka_transactions/)
...
### ⏳ Pending
- Phase 1: Backend Infrastructure (Next: database schema, settlement models)
